...
for i in range(learntNumber):
    plt.plot(smoothSparseCurve([t[i] for t in joinGroupDateLearntHist]),color = 'g')
try:
...
for i in range(learntNumber):
    plt.plot(smoothSparseCurve([t[i] for t in timeInGroupLearntHist]),color = 'g')
try:
    plt.plot(smoothSparseCurve([min(t[i] for i in range(adultsNumber)) for t in timeInGroupAdultsHist]),color = 'b')
except ValueError:
    print('No Adults')
plt.title('time in the group, in red for learnersi, in green for old fishes and in blue for min in non learners')
plt.show()
...
subHist=[]
for f in range(adultsNumber):
    plt.plot(posHistoryA[f][ runDuration - maxPlot : runDuration - 1],color='b')
for f in range(learnersNumber):
    plt.plot(posHistoryL[f][runDuration - maxPlot : runDuration - 1],'ro',color='r')
for f in range(learntNumber):
    plt.plot(posHistoryLearnt[f][runDuration - maxPlot : runDuration - 1],'ro',color='g')
